...
function buildFullProfile(profile, authUserMetadata) {
    return {
        ...profile,
...
router.post('/login', async (req, res) => {
    try {
        ...
        const tempSupabase = (0, supabase_js_1.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE, {
...
router.post('/register', async (req, res) => {
    try {
        ...
        const token = authHeader.split(' ')[1];
        const { data: { user: adminUser }, error: adminError } = await SupabaseClient_1.supabase.auth.getUser(token);
        if (adminError || !adminUser) {
            return res.status(401).json({ error: 'Token inválido' });
        }
        // Verificar se é super_admin
        const { data: adminProfile } = await SupabaseClient_1.supabase
            .from('profiles')
            .select('role')
            .eq('id', adminUser.id)
            .single();
        ...
        if (profileError) {
            ...
            await SupabaseClient_1.supabase.auth.admin.deleteUser(authData.user.id);
...
router.get('/team', async (req, res) => {
    try {
        ...
        const { data: profiles, error } = await SupabaseClient_1.supabase
            .from('profiles')
            .select('*')
        ...
        if (profiles && profiles.length > 0) {
            await Promise.all(profiles.map(async (p) => {
                const { data: userData } = await SupabaseClient_1.supabase.auth.admin.getUserById(p.id);
...
router.delete('/team/:id', async (req, res) => {
    try {
        ...
        // Deletar auth user
        const { error: authError } = await SupabaseClient_1.supabase.auth.admin.deleteUser(id);
        if (authError) {
...
// ============================================
router.patch('/team/:id/password', async (req, res) => {
    try {
        const authHeader = req.headers.authorization;
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
            return res.status(401).json({ error: 'Token não fornecido' });
        ...
        const { data: targetUser, error: targetError } = await SupabaseClient_1.supabase.auth.admin.getUserById(id);
        if (targetError || !targetUser.user) {
            return res.status(404).json({ error: 'Usuário não encontrado' });
        }
        const currentMetadata = targetUser.user.user_metadata || {};
        // Atualizar senha no Auth
        const { error: updateError } = await SupabaseClient_1.supabase.auth.admin.updateUserById(id, {
            password
        });
        if (updateError) {
...
router.patch('/team/:id', async (req, res) => {
    try {
        ...
        const { error: authError } = await SupabaseClient_1.supabase.auth.admin.updateUserById(id, updateData);
        if (authError) {
